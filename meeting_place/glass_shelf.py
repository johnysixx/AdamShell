...
class GlassShelf:
    ...
    def appear_shared_glass(self, kind):
        allowed_kinds = {"generic", "beer_mug", "shot_glass"}
        if kind not in allowed_kinds:
            raise ValueError(f"Unknown shared glass kind: {kind}")
        kind_number = sum(
            1
        ...
        glass = BarGlass(
            ...
            dirt=0.0,
            location=self.name,
            capacity_litres=0.5 if kind == "beer_mug" else None,
        )
        self.shared_glasses.append(glass)
        UniverseLogger.boot(
            f"SHARED {kind.upper()} APPEARS: {glass.name}"
        )
        return glass
    ...
    def register_policy_decision(self, decision):
        ...
        if glass_mode == "personal_named":
            return self._create_personal_glass(entity_id)
        if glass_mode == "reserved_unnamed":
            return self._create_reserved_glass(entity_id)
        if glass_mode == "shared":
            return self._create_shared_glass(entity_id)
        return None
    ...
    def _create_personal_glass(self, entity_id):
        ...
        UniverseLogger.boot(f"PERSONAL GLASS CREATED: {glass.name}")
        return glass

    def _create_reserved_glass(self, entity_id):
        for glass in self.reserved_glasses:
            if glass.reserved_for == entity_id:
                return glass
        reservation_number = len(self.reserved_glasses) + 1
        glass = BarGlass(
            name=None,
        ...
        UniverseLogger.boot(
            "UNNAMED RESERVED GLASS CREATED: "
    ...
    def _create_shared_glass(self, entity_id):
        ...
        self.shared_glasses.append(glass)
        UniverseLogger.boot(f"SHARED GLASS CREATED: {glass.name}")
        return glass
