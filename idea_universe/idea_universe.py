...
class IdeaUniverse:
    ...
    def __init__(self, universe):
        ...
        registry = getattr(self.universe, 'universe_registry', None)
        if registry is None:
            raise RuntimeError('Idea Universe requires UniverseRegistry')
        self.universe_id = registry.register_universe(name='idea_universe', universe_type='pre_physical_idea_reality')
        self.primordial_waters = PrimordialWaters()
        self.primordial_nebula = None
        self.starry_sky = []
        self.state = {'name': 'idea_universe', 'type': 'pre_physical_idea_reality', 'state': 'created', 'part_of_physics': False, 'primordial_waters': self.primordial_waters, 'primordial_nebula': None, 'entities': self.entities, 'events': self.events}
    ...
    def create_primordial_star(self):
        if not self.stellar_epoch_started:
            raise RuntimeError('Primordial idea star requires stellar epoch.')
    ...
    def run_primordial_stellar_epoch(self, star_count=1):
        if not self.stellar_epoch_started:
            raise RuntimeError('Primordial stellar epoch has not started.')
        stars = []
        remnants = []
        ...
        self.primordial_nebula = PrimordialNebula(source_remnants=remnants)
        self.state['primordial_nebula'] = self.primordial_nebula
        return {'primordial_nebula': self.primordial_nebula, 'stars': stars, 'remnants': remnants}
    ...
    def add_entity(self, entity):
        ...
        print(f"IDEA UNIVERSE ENTITY BORN: {getattr(entity, 'name', None)}")
