...
class CatGroupMythSystem:
    ...
    def retell(self, source_group_id, target_group_id, myth_id, transformation=None):
        ...
        root_myth_id = getattr(copied, 'lineage_root', myth_id)
        ...
        copied.parent_version = parent_myth_id if new_myth_id != myth_id else getattr(copied, 'parent_version', None)
        copied.generation = int(getattr(copied, 'generation', 0)) + (1 if new_myth_id != myth_id else 0)
        copied.retellings += 1
    ...
    def verify_against_knowledge(self, group_id, myth_id):
        ...
        myth.verified = bool(getattr(knowledge, 'verified', False))
        if myth.verified:
            myth.credibility = self._clamp(max(myth.credibility, float(getattr(knowledge, 'confidence', 0.0))))
