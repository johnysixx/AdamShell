...
class KittenBirthResolver:
    ...
    def __init__(self, universe):
        ...
        self.history = []
        self.family_system = CatFamilySystem()
        self.biology_gate = PhysicalBiologyGate(universe)
        self.development_resolver = CatDevelopmentResolver(universe)
    ...
    def _next_kitten_name(self):
        ...
        if cats_layer is None:
            raise RuntimeError('Kitten birth requires cats_layer.')
        existing_names = {cat.name for cat in cats_layer.cats}
        number = 1
        while True:
            name = f'kitten_{number:04d}'
