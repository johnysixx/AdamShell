...
class KittenEmbryoResolver:
    ...
    def create_embryo(
        ...
        embryo_id = (
            f"embryo_{self.embryo_count:04d}"
        )

        genotype = (
            genotype_override
            if genotype_override is not None
            else CatGenotype.inherit(
                mother_genotype=mother[
                    "genotype"
                ],
                father_genotype=father[
                    "genotype"
                ],
                rng=rng
            )
        ...
        if not viability["viable"]:
            cronenberg = (
                ...
                .create_cronenberg_from_quantum_error(
                    error=RuntimeError(
                        "Nonviable kitten genotype "
                        f"replaced embryo {embryo_id}."
                    ),
                    source_component=(
                        "kitten_embryo_resolver"
                    ),
                    source_operation=(
                        "nonviable_embryo"
                    )
    ...
    def _validate_parents(
        ...
        if mother.get("sex") != "female":
            raise ValueError(
                "Embryo mother must be female."
            )

        if father.get("sex") != "male":
            raise ValueError(
                "Embryo father must be male."
            )

        if "genotype" not in mother:
            raise ValueError(
                "Mother has no genotype."
            )

        if "genotype" not in father:
            raise ValueError(
                "Father has no genotype."
            )