    ...
    def select_father(self, mating_contacts, rng):
        if not all(
            ...
            raise TypeError(
                ...
                "CatMatingContact objects."
            )

        successful_contacts = [contact for contact in mating_contacts if contact.successful and contact.male_ref is not None]
        if not successful_contacts:
            raise ValueError('Paternity selection requires at least one successful contact.')
        weighted_fathers = [contact.male_ref for contact in successful_contacts]
