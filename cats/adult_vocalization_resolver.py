...
class AdultVocalizationResolver:
    ...
    def teach(self, teacher, kitten, vocalization, current_day):
        ...
        teacher_learning = getattr(teacher, 'learning', {})
        kitten_learning = getattr(kitten, 'learning', {})
        teacher_skill = teacher_learning.get('skills', {}).get('adult_meowing', {})
        kitten_skill = kitten_learning.get('skills', {}).get('adult_meowing', {})
        if not teacher_skill.get('learned', False):
            return self._deny(teacher, kitten, vocalization, current_day, 'teacher_does_not_know_adult_meowing')
        teacher_vocalizations = teacher_skill.get('vocalizations', {})
        ...
        kitten_vocalizations = kitten_skill.setdefault('vocalizations', {name: False for name in CatLearning.ADULT_VOCALIZATIONS})
        ...
        if completed:
            kitten_skill.update({'learned': True, 'progress': 1.0, 'teacher': teacher.name, 'learned_on_day': current_day})
