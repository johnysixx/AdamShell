...
class CatGroupSuccessionSystem:
    ...
    def __init__(self, group_system):
        ...
        self.roles = CatGroupRoleSystem(group_system)

    def candidates(self, group_id, role, cats, exclude=None):
        ...
        for cat in members:
            if cat.name in excluded:
                continue
            check = self.roles.suitability(group_id, cat, role)
            if not check['eligible']:
                continue
            score = float(check['score']) + min(0.15, int(cat.group.group_events) * 0.005) + float(cat.group.influence) * 0.1
            ranked.append({'cat': cat, 'cat_name': cat.name, 'score': round(min(1.0, score), 4)})
        ranked.sort(key=lambda item: (item['score'], item['cat_name']), reverse=True)
        return ranked

    def succeed(self, group_id, vacated_cat, role, cats, reason='role_vacated'):
        group = self.group_system._group(group_id)
        holders = group.roles.get(role, [])
        was_holder = vacated_cat.name in holders
        if was_holder:
            self.roles.release(group_id, vacated_cat, role, reason=reason)
        ranked = self.candidates(group_id, role, cats, exclude=[vacated_cat.name])
        if not ranked:
            self._weaken_dependent_institutions(group, role, amount=0.2)
            event = {'name': 'cat_group_role_became_vacant', 'group_id': group_id, 'role': role, 'previous_holder': vacated_cat.name, 'reason': reason, 'successor': None, 'succeeded': False}
            group.succession_history.append(deepcopy(event))
    ...
    def _weaken_dependent_institutions(self, group, role, amount):
        for institution in group.institutions.values():
            if role not in getattr(institution, 'roles', []):
                continue
            institution.continuity = max(0.0, float(getattr(institution, 'continuity', 1.0)) - amount)
            if institution.continuity <= 0.1:
                institution.active = False

    def _strengthen_dependent_institutions(self, group, role, amount):
        for institution in group.institutions.values():
            if role not in getattr(institution, 'roles', []):
                continue
            institution.continuity = min(1.0, float(getattr(institution, 'continuity', 1.0)) + amount)
