...
class FelineTeacherResolver:
    ...
    def find_teachers(self, student, ability_name, cats=None):
        ...
        awareness = wisdom.awareness_record(ability_name)
        ...
        known_teacher_names = list(awareness.known_teachers)
        candidates = []
        teachers = []
        for teacher_name in known_teacher_names:
            teacher = self._find_cat(cats=cats, cat_name=teacher_name)
        ...
        return self._result(student=student, ability_name=ability_name, reason=reason, candidates=candidates, teachers=teachers)

    def choose_teacher(self, student, ability_name, method_name=None, cats=None):
        search = self.find_teachers(student=student, ability_name=ability_name, cats=cats)
    ...
    def request_lesson(self, student, ability_name, ability_resolver, method_name=None, cats=None):
        choice = self.choose_teacher(student=student, ability_name=ability_name, method_name=method_name, cats=cats)
        if not choice['chosen']:
            return {'name': 'feline_ability_lesson_request_failed', 'student': student.name, 'ability': ability_name, 'requested_method': method_name, 'reason': choice['reason'], 'learned': False}
    ...
    def _resolve_cats(self, cats):
        if cats is not None:
            return list(cats)
        cats_layer = getattr(self.universe, 'cats_layer', None)
        if cats_layer is None:
            return []
        return list(getattr(cats_layer, 'cats', []))

    def _find_cat(self, cats, cat_name):
        for cat in cats:
            if cat.name == cat_name:
                return cat
        return None

    def _result(self, student, ability_name, reason, candidates, teachers):
        event = {'name': 'feline_ability_teacher_search', 'student': getattr(student, 'name', None), 'ability': ability_name, 'reason': reason, 'candidates': candidates, 'teachers': teachers, 'teacher_count': len(teachers), 'found': bool(teachers)}
        self._record(event)
        return event
