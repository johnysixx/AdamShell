...
class CatGroupSystem:
    ...
    def __init__(self, cats_layer=None):
        ...
        self.groups = {}
        self.territory_system = CatTerritorySystem(cats_layer)

    def create_group(self, founder, name=None):
        self._require_cat(founder)
    ...
    def respond_to_threat(self, group_id, cats, threat):
        ...
        for member in members:
            ...
            if defense_score >= 0.55:
                member.state = 'group_defending'
                member.group.defense_events += 1
                defenders.append(member.name)
            else:
    ...
    def same_group(self, first, second):
        ...
        return bool(first.group.member and second.group.member and (first.group.group_id == second.group.group_id))

    def _set_membership(self, cat, group_id, joined_order):
        cat.group.group_id = group_id
        cat.group.member = True
        cat.group.joined_order = joined_order
        cat.group.group_events += 1

    def _member_objects(self, group, cats):
    ...
    def _group(self, group_id):
        ...
        if group is None:
            raise KeyError(f'Unknown cat group: {group_id}')
        return group

    def _record(self, group, event, cats):
    ...
    def _require_cat(self, cat):
        if not isinstance(cat, Cat):
            raise TypeError('CatGroupSystem requires Cat.')
