...
class CatGroupSanctionSystem:
    ...
    def sanction(self, group_id, cat, violation):
        group = self.group_system._group(group_id)
        if not isinstance(violation, CatViolation):
            raise TypeError('CatGroupSanctionSystem requires CatViolation.')
        severity = float(violation.severity)
        previous = len(cat.norms['sanctions'])
        effective = min(1.0, severity + previous * 0.08)
        if effective < 0.25:
            ...
            sanction_type = 'expulsion_recommended'
        event = {'name': 'cat_group_sanction', 'group_id': group_id, 'cat': cat.name, 'sanction': sanction_type, 'severity': round(effective, 4), 'source_violation': deepcopy(violation)}
        self._apply(group, cat, sanction_type, effective)
        group.sanction_history.append(deepcopy(event))
        cat.norms['sanctions'].append(deepcopy(event))
        return event

    def _apply(self, group, cat, sanction_type, severity):
        if sanction_type == 'warning':
            cat.norms['warnings'] += 1
        ...
        elif sanction_type == 'expulsion_recommended':
            cat.state = 'group_expulsion_recommended'
