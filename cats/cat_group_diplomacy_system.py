...
from cats.cat_group_memory_system import CatGroupMemorySystem

class CatGroupDiplomacySystem:

    def __init__(self, group_system):
        self.group_system = group_system
        self.memory = CatGroupMemorySystem(group_system)
    ...
    def mutual_relation(self, first_group_id, second_group_id):
        first = self.evaluate(first_group_id, second_group_id)
        second = self.evaluate(second_group_id, first_group_id)
        return {'first': first, 'second': second, 'mutual_score': round((first['score'] + second['score']) / 2.0, 4)}
