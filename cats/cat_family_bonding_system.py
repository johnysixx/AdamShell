...
class CatFamilyBondingSystem:
    ...
    def evaluate(
        ...
        care_events = max(
            int(
                first.maternal_care_received.get(
                    "care_events",
                    0
                )
            ),
            int(
                second.maternal_care_received.get(
                    "care_events",
                    0
                )
            )
        ...
        parent_child = relation in {
            "mother",
            "father",
            "child"
        }

        siblings = relation in {
            "sibling",
            "half_sibling",
            "sibling_littermate",
    ...
    def form_bond(
        ...
        check_first = self.evaluate(
            first,
            second
        )

        check_second = self.evaluate(
            second,
            first
        )

        if (
            not check_first["eligible"]
            or not check_second["eligible"]
        ):
            return {
                "name": "cat_family_bond_not_formed",
                "first": first.name,
                "second": second.name,
                "formed": False,
                "reason": (
