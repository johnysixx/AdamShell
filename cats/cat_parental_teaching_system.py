...
class CatParentalTeachingSystem:
    ...
    def teach(self, parent, kitten, skill, progress=0.25, current_day=None):
        self._require_cat(parent)
        self._require_cat(kitten)
        role = self._parent_role(parent, kitten)
        if role is None:
            return {'name': 'parental_teaching_denied', 'parent': parent.name, 'kitten': kitten.name, 'reason': 'not_parent', 'taught': False}
    ...
    def _record(self, parent, kitten, skill, role, current_day):
        state = kitten.parental_teaching
        state.lessons_received += 1
        state.last_lesson = skill
        state.last_teacher = parent.name
        teachers = state.teachers
        teachers[parent.name] = int(teachers.get(parent.name, 0)) + 1
        skills = state.skills
    ...
    def _require_cat(self, cat):
        if not isinstance(cat, Cat):
            raise TypeError('CatParentalTeachingSystem requires Cat.')
