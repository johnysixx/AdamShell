...
class Day0FirstBarShiftTests(unittest.TestCase):
    ...
    def test_shift_exists_before_first_guest(self):
        self.scene.start_shift()
        self.assertTrue(self.bar.bartender.shift_active)
        self.assertEqual(self.bar.entities, [])

    def test_serpent_arrives_at_0420_bar_time(self):
        self.scene.start_shift()
        self.scene.serpent_is_born_and_enters()
        self.assertEqual(self.bar.bar_clock.hour, 4)
        self.assertEqual(self.bar.bar_clock.minute, 20)
        self.assertEqual(self.bar.bar_clock.time_text, '04:20')
        arrival = next((event for event in self.scene.history if event.get('name') == 'serpent_entered_bar'))
        self.assertEqual(arrival['bar_time'], '04:20')

    def test_serpent_has_three_drinks_and_unpaid_tab(self):
        self.scene.start_shift()
        self.scene.serpent_is_born_and_enters()
    ...
    def test_bartender_refuses_serpent_bet(self):
        ...
        result = self.scene.serpent_proposes_bet()
        self.assertFalse(result['response']['accepted'])

    def test_requested_checkpoint(self):
        state = self.scene.advance_to_lilith_entry()
        self.assertTrue(state['shift_active'])
        self.assertTrue(state['serpent']['in_bar'])
        self.assertEqual(state['serpent']['bar_state']['activity'], 'tasting_ordered_drinks')
        ...
        self.assertLess(history_names.index('serpent_born'), history_names.index('god_born'))
        self.assertLess(history_names.index('god_begins_first_unnamed_book'), history_names.index('lilith_born'))

    def test_canonical_lilith_entry_checkpoint(self):
        state = self.scene.advance_to_lilith_entry()
        self.assertEqual(self.bar.bar_clock.time_text, '04:20')
        self.assertTrue(state['shift_active'])
        serpent = self.scene.serpent
        self.assertIsNotNone(serpent)
        self.assertIn(serpent, self.bar.entities)
        self.assertEqual(serpent.bar_state['seat'], 'at_bar')
        self.assertEqual(serpent.bar_state['activity'], 'tasting_ordered_drinks')
        self.assertEqual(serpent.bar_state['drinks'], ['wine', 'beer', 'mead'])
        self.assertEqual(serpent.bar_state['tab'], 'open')
        self.assertFalse(serpent.bar_state['paid'])
        ...
        self.assertEqual(lilith.type, 'idea_entity')
        self.assertEqual(lilith.principle['name'], 'feminine_principle')
        self.assertFalse(hasattr(lilith, 'met_god'))
        self.assertNotIn('god', getattr(lilith, 'knowledge', {}))
        names = [event['name'] for event in state['history']]
        expected_order = ['bartender_shift_started', 'serpent_born', 'serpent_entered_bar', 'serpent_orders_wine_beer_and_mead', 'serpent_proposes_bet', 'bartender_refused_bet', 'god_born', 'god_entered_library', 'god_begins_first_unnamed_book', 'lilith_born', 'lilith_entered_bar']
        self.assertEqual(names, expected_order)
        self.assertEqual(len(self.bar.entities), 2)
        self.assertIs(self.bar.entities[0], serpent)
        self.assertIs(self.bar.entities[1], lilith)
