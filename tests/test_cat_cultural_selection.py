...
from cats.cat_group_culture_system import CatGroupCultureSystem
from cats.cat_cultural_adoption_system import CatCulturalAdoptionSystem
from cats.cat_group_cultural_conflict_system import CatGroupCulturalConflictSystem
from cats.cat_group_knowledge_system import CatGroupKnowledgeSystem
from cats.cat_group_myth_system import CatGroupMythSystem
from cats.cat_group_innovation_system import CatGroupInnovationSystem
from cats.cat_memetic_selection_system import CatMemeticSelectionSystem

class CatCulturalSelectionTests(unittest.TestCase):
    ...
    def test_conflicting_preferences_create_friction(self):
        culture = CatGroupCultureSystem(self.groups)
        culture.express_preference(self.first_group, 'sleeping_place', 'bar_cloth', strength=0.8)
    ...
    def _create_myth_and_innovation(self):
        ...
        knowledge.contribute(self.first_group, self.first, 'safe_route', {'route': 'bar_to_library'}, 'navigation', confidence=1.0)
        knowledge.contribute(self.first_group, self.first, 'danger_scent', {'aroma': 'cronenberg'}, 'danger', confidence=1.0)
        myths = CatGroupMythSystem(self.groups)
    ...
    def test_memetic_selection_distinguishes_survival(self):
        ...
        selection = CatMemeticSelectionSystem(self.groups)
        selection.expose_myth(self.first_group, self.cats.cats, myth_id)
