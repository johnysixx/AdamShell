...
from cats.kitten_birth_resolver import KittenBirthResolver
...
class AlternatingFatherRng:
    ...
    def choice(self, values):
        values = list(values)
        if values and getattr(values[0], 'type', None) == 'cat':
            value = values[self.index % len(values)]
            self.index += 1
            return value
        return values[0]

class KittenBirthResolverTests(unittest.TestCase):
