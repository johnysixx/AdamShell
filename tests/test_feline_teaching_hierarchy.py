...
class FelineTeachingHierarchyTests(unittest.TestCase):
    ...
    def teach_pazuzu_to_teach(self):
        return self.resolver.teach_method(teacher=self.garfield, student=self.pazuzu, ability_name='teach_other_cats', method_name='garfield_teaching_method')

    def teach_pazuzu_meta_teaching(self):
        return self.resolver.teach_method(teacher=self.garfield, student=self.pazuzu, ability_name='teach_teaching', method_name='garfield_meta_teaching_method')

    def test_garfield_can_teach_any_cat_to_teach(self):
        result = self.teach_pazuzu_to_teach()
        self.assertTrue(result['learned'])
        ability = self.pazuzu.feline_wisdom['abilities']['teach_other_cats']
        self.assertTrue(ability['learned'])

    def test_teacher_can_teach_own_ability(self):
        self.teach_pazuzu_to_teach()
        result = self.resolver.teach_method(teacher=self.pazuzu, student=self.foreign_cat, ability_name='open_human_door', method_name='hang_on_handle')
        self.assertTrue(result['learned'])
    ...
    def test_forbidden_foreign_teacher_creation_makes_cronenberg(self):
        self.teach_pazuzu_to_teach()
        previous_count = self.universe.cronenberg_count
        result = self.resolver.teach_method(teacher=self.pazuzu, student=self.foreign_cat, ability_name='teach_other_cats', method_name='garfield_teaching_method')
    ...
    def test_meta_teacher_can_create_foreign_teacher(self):
        ...
        result = self.resolver.teach_method(teacher=self.pazuzu, student=self.foreign_cat, ability_name='teach_other_cats', method_name='garfield_teaching_method')
        self.assertTrue(result['learned'])
        self.assertEqual(self.universe.cronenberg_count, previous_count)

    def test_untrained_cat_cannot_teach_foreign_cat(self):
        result = self.resolver.teach_method(teacher=self.foreign_cat, student=self.pazuzu, ability_name='open_human_door', method_name='hang_on_handle')
        self.assertFalse(result['learned'])
