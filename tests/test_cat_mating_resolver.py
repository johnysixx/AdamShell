...
class MultiSireRng:
    ...
    def choice(self, values):
        values = list(values)
        if values and isinstance(values[0], Cat) and (values[0].type == 'cat'):
            value = values[self.next_father % len(values)]
            self.next_father += 1
            return value
        return values[0]

class CatMatingResolverTests(unittest.TestCase):
    ...
    def test_neutered_male_cannot_be_candidate(self):
        self.first_male.reproduction = CatReproduction.create_state(sex='male', neutered=True)
        with self.assertRaises(ValueError):
            self.resolver.mate(self.female, self.first_male)
