...
class CatMEOWBarInvitationTests(unittest.TestCase):

    def setUp(self):
        self.universe = Universe()
        self.universe.universe_registry = UniverseRegistry()
        self.meeting_place = MeetingPlace(self.universe)
    ...
    def _make_right_human(self):
        for _ in range(8):
            self.bonds.remember_interaction(self.cat, self.human, positive=True, significance=0.15)

    def _understood_invitation(self):
    ...
    def test_unknown_human_is_not_MEOWed(self):
        invitations = CatMeowInvitationSystem(self.cats)
