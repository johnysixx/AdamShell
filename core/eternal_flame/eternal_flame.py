...
class EternalFlame:
    ...
    def ignite(
        ...
        if not isinstance(source_idea, dict):
            raise TypeError(
                "Eternal Flame requires an idea source."
            )

        if source_idea.get("name") != "eternal_fire":
            raise ValueError(
                "Invalid idea source for Eternal Flame."
            )

        self.source_idea = {
            "name": source_idea.get("name"),
            "type": source_idea.get("type"),
            "state": source_idea.get("state")
        }
