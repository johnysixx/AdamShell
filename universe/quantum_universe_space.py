...
class QuantumUniverseSpace:
    ...
    def find_cat_route(self, cat_id):
        return next(
            (
                route
                for route in self.cat_routes
                if route.cat_id == cat_id
                and route.observation_active
            ),
            None
        )
    ...
    def advance_cat_route(
        ...
        if next_position is None:
            return {
                "result": "already_arrived"
            }

        crossing_cronenbergs = [
            cronenberg
            for cronenberg in cronenbergs
            if getattr(cronenberg, "is_alive", False)
            and route.position_matches(
                getattr(cronenberg, "position", {})
            )
        ]
        ...
        if crossing_cronenbergs:
            ...
            previous_detour_count = route.detour_count_for(
                cronenberg_position
            )
            ...
            if previous_detour_count > 0:
                def repeated_cat_route_paradox():
                    raise RuntimeError(
                        "Cat route paradox: repeated obstacle "
                        "on shortest path."
                    )

                quantum_error = (
                    universe.quantum_error_boundary.execute(
                        operation=(
                ...
                manifested_cronenberg = quantum_error.get(
                    "cronenberg"
                )
                ...
                if manifested_cronenberg is not None:
                    ...
                    cronenberg.quantum_link_system.add_link(
                        target_id=manifested_cronenberg.id,
                        link_type=(
                    ...
                    manifested_cronenberg.quantum_link_system.add_link(
                        target_id=cronenberg.id,
            ...
            encounter = encounter_system.resolve(
                cat=cat,
